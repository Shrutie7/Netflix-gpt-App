...
dispatch((removeGptMovieResults()))
  ...
  const popover = (
    <Popover id="popover-basic">
      <Popover.Body>
      <div className="bg-black bg-opacity-80 border border-solid border-neutral-100 rounded-lg w-48 mt-5 pt-4">
      <div className="text-white ml-4 mt-1">
      <div className="flex">
      <img className="w-6 h-6 text-lg font-semibold" src={user?.photoURL} alt="usericon" />&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <hr className="bg-slate-500 mt-1"></hr>
  ...
  return (
    <div className="absolute w-full px-8 py-2 bg-gradient-to-b from-black z-10 flex flex-col md:justify-between md:flex-row">
      ...
      {user && (
        <div className="m-auto flex gap-6 pt-2 md:m-0">
      ...
      SUPPORTED_LANGUAGES.map((lang) => (
        ...
        {!showgptsearch?<button className="bg-red-800 cursor-pointer rounded-md p-2 font-semibold text-white md:h-10 pt-1"  onClick={()=>handleGPTsearchclick()}>GPT Search</button>:<button className="bg-red-800 cursor-pointer rounded-md p-2 font-semibold text-white md:h-10 pt-2" onClick={()=>handleGPTsearchclick()}>Home</button>}
          {/* <p className="text-white">Welcome {user?.displayName}</p> */}
          <img className="hidden md:block w-9 h-9" src={user?.photoURL} alt="usericon" />
